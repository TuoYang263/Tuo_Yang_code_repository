...
public:
	...
	KCFTracker(bool hog = true,                 // 使用hog特征
		bool fixed_window = false,       //使用固定窗口大小
	...
	bool _update(Mat&,Rect2f&,float);  //用于决定是否要更新

	float _peak_val_threshold = 0.28;			//判断跟踪失败的阈值    //const
...
protected:
	...
	//计算跟踪过程中的中心位置误差和边界框重叠率
	void getCentralErrorAndOverlapRate(const cv::Rect&, const cv::Rect&, float*, float*);
